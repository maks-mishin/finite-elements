...
Matrix operator*(const Matrix& one, const Matrix& two) {
    if (one.GetNumRows() != two.GetNumColumns()) {
        throw invalid_argument("Mismatched number of rows one with number of columns two");
    }
...
Matrix operator*(const Matrix& one, double lambda) {
    Matrix result(one.GetNumRows(), one.GetNumColumns());

    for (int i = 0; i < result.GetNumRows(); ++i) {
        for (int j = 0; j < result.GetNumColumns(); ++j) {

            result.At(i, j) = one.At(i, j) * lambda;
        }
    }
    return result;
}
